...
def softmax_loss(x, y):
    ...
    loss = np.exp(x[range(N), y]) / np.sum(np.exp(x), axis=1)
    loss = np.sum(-np.log(loss)) / len(y)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, batch_mean, batch_std, gamma) 
        ...
        cache = (x, running_mean, running_std, gamma)  
...
def batchnorm_backward(dout, cache):
    ...
    x, batch_mean, batch_std, gamma  = cache
    N = x.shape[0]
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum((x - batch_mean) / batch_std * dout, axis=0)

    x_ = (x - batch_mean) / batch_std
    dx_ = dout * gamma
    dvar  = -0.5 * np.sum(dx_ * (x - batch_mean) * np.power(batch_std, -3.0), axis=0)
    dmean = (np.sum(dx_ * -1.0 / batch_std, axis=0) + dvar * np.sum(-2.0 * (x - batch_mean), axis=0) / N)  
    dx = dx_ * 1.0 / batch_std + dvar * 2.0 * (x - batch_mean) / N + dmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum((x - batch_mean) / batch_std * dout, axis=0)
    dx = (
        (gamma * np.power(batch_std, -1.0) / N) * 
        (dout * N -np.sum(dout, axis = 0) -
      (x - batch_mean) * np.power(batch_std, -2.0) * np.sum(dout * (x - batch_mean), axis=0)
    )
  )
        ...
        for f in range(F):
            for h_ in range(H_):
                for w_ in range(W_):
                    ...
                    out[n, f, h_, w_] = np.sum(window * w[f,:,:,:]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        dx_pad = np.pad(dx[n,:,:,:], ((0,0),(pad,pad),(pad,pad)), 'constant', constant_values=0)
        x_pad = np.pad(x[n,:,:,:], ((0,0),(pad,pad),(pad,pad)), 'constant', constant_values=0)
        for f in range(F):
            for h_ in range(H_):
                for w_ in range(W_):
                    ...
                    w1 = w_ * stride
                    w2 = w_ * stride + WW
                    dx_pad[:, h1:h2, w1:w2] += w[f,:,:,:] * dout[n,f,h_,w_]
                    dw[f,:,:,:] += x_pad[:, h1:h2, w1:w2] * dout[n,f,h_,w_]
                    db[f] += dout[n,f,h_,w_]
        dx[n,:,:,:] = dx_pad[:,1:-1,1:-1]
