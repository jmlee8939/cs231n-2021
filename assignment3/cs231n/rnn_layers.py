...
def rnn_step_backward(dnext_h, cache):
    ...
    d_tanh = dnext_h * (1 - next_h) * (1 + next_h)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T) :
        h0, cache_ = rnn_step_forward(x[:,i,:], h0, Wx, Wh, b)
        h[:,i,:] = h0
        cache.append(cache_)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    W_xh = np.concatenate((Wx, Wh), axis = 0)
    gates = np.dot(xh, W_xh) + b
     
    i_t = sigmoid(gates[:, :H])
    f_t = sigmoid(gates[:, H:2*H])
    o_t = sigmoid(gates[:, 2*H:3*H])
    C_t = np.tanh(gates[:, 3*H:4*H])
    ...
    cache = (i_t, f_t, o_t, C_t, prev_c, prev_h, next_c, x, Wx, Wh) 
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    i_t, f_t, o_t, C_t, prev_c, prev_h, next_c, x, Wx, Wh = cache 
    ...
    di_t = i_t * (1 - i_t) * di_t
    df_t = f_t * (1 - f_t) * df_t
    dC_t = (1 - C_t) * (1 + C_t) * dC_t
    do_t = o_t * (1 - o_t) * do_t

    dWh_i = np.dot(di_t.T, prev_h).T
    dWh_f = np.dot(df_t.T, prev_h).T
    dWh_C = np.dot(dC_t.T, prev_h).T
    dWh_o = np.dot(do_t.T, prev_h).T

    dWh = np.concatenate((dWh_i, dWh_f, dWh_o, dWh_C), axis = 1)

    dWx_i = np.dot(x.T, di_t)
    dWx_f = np.dot(x.T, df_t)
    dWx_C = np.dot(x.T, dC_t)
    dWx_o = np.dot(x.T, do_t)

    dWx = np.concatenate((dWx_i, dWx_f, dWx_o, dWx_C), axis = 1)

    dprev_h = np.dot(di_t, Wh[:, :H].T) + np.dot(df_t, Wh[:, H:2*H].T) + np.dot(do_t, Wh[:, 2*H:3*H].T) + np.dot(dC_t, Wh[:, 3*H:4*H].T)
    dx = np.dot(di_t, Wx[:, :H].T) + np.dot(df_t, Wx[:, H:2*H].T) + np.dot(do_t, Wx[:, 2*H:3*H].T) + np.dot(dC_t, Wx[:, 3*H:4*H].T)

    db = np.concatenate((di_t, df_t, do_t, dC_t), axis = 1).sum(axis=0)
...
def lstm_backward(dh, cache):
    ...
    for i in np.arange(T - 1, -1, -1):
        dnext_h += dh[:, i, :]
        cache_ = cache[i]
        dx[:, i, :], dprev_h, dprev_c, dWx_, dWh_, db_ = lstm_step_backward(dnext_h, dnext_c, cache_)
        dWx += dWx_
        dWh += dWh_
        db += db_
