...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.dot(z_i, z_j) / (torch.linalg.vector_norm(z_i) * torch.linalg.vector_norm(z_j))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N) :
            if i == k :
                de2 += np.exp(sim(z_k_N, out[i]) / tau)
            elif i == k + N :
                de1 += np.exp(sim(z_k, out[i]) / tau)
            else :
                de1 += np.exp(sim(z_k, out[i]) / tau)
                de2 += np.exp(sim(z_k_N, out[i]) / tau)

        temp_loss = - np.log(np.exp(sim(z_k, z_k_N)/tau) / de1)
        temp_loss += - np.log(np.exp(sim(z_k_N, z_k)/tau) / de2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs /= torch.linalg.norm(out_left, dim=1)
    pos_pairs /= torch.linalg.norm(out_right, dim=1)
...
def compute_sim_matrix(out):
    ...
    sim_matrix /= torch.outer(torch.linalg.norm(out, dim=1), torch.linalg.norm(out, dim=1))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim = sim_positive_pairs(out_left, out_right).to(device)
    ...
    numerator = torch.tile(numerator, (2, 1))
