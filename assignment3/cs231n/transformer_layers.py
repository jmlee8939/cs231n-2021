...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        dim_ = 10000**(-torch.arange(0, embed_dim, 2)/embed_dim)
        len_ = torch.arange(0, max_len, 1)

        even_idx = torch.arange(0, embed_dim, 2)
        odd_idx = even_idx + 1 
        pe[0, :, even_idx] = torch.sin(torch.outer(len_, dim_))
        pe[0, :, odd_idx] = torch.cos(torch.outer(len_, dim_))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        k = self.key(key).view(N, -1, self.h, self.d_k) # N, T, H, E/H
        q = self.query(query).view(N, -1, self.h, self.d_k) # N, S, H, E/H
        v = self.value(value).view(N, -1, self.h, self.d_k) # N, T, H, E/H 

        q = q.transpose(1, 2) # N, H, T, E/H
        k = k.transpose(1, 2) # N, H, S, E/H
        v = v.transpose(1, 2) # N, H, T, E/H

        scores = torch.matmul(q, k.transpose(2, 3)) / math.sqrt(self.d_k)

        if attn_mask is not None :
            attn_mask = F.pad(attn_mask, (0, (T-S)), mode = 'constant', value = 0)
        ...
        concat = scores.transpose(1,2).contiguous().view(N, S, D)
