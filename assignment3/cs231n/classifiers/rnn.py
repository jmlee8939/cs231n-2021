...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        dh, grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dx, cache4)
        if self.cell_type == 'rnn' :
    ...
    def sample(self, features, max_length=30):
        ...
        idx = np.array(self._start * np.ones(N), dtype = 'int32')
        c_ = np.zeros_like(h_)
        ...
        for i in range(max_length) :
            ...
            caption, _ = affine_forward(h_, W_vocab, b_vocab)
            idx = np.argmax(caption, axis=-1)
            captions[:, i] = idx
