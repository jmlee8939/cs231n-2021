...
def compute_saliency_maps(X, y, model):
    ...
    y_pred = model(X)
    score = y_pred.gather(1, y.view(-1, 1)).squeeze()
    score = torch.sum(score)
    score.backward()

    saliency = X.grad.data
    ch_1 = torch.abs(saliency[:, 0, :, :])
    ch_2 = torch.abs(saliency[:, 1, : ,:])
    ch_3 = torch.abs(saliency[:, 2, :, :])
    saliency = ch_1 + ch_2 + ch_3

    for i in range(N) :
        saliency[i] -= torch.min(saliency[i])
        saliency[i] /= torch.max(saliency[i])
